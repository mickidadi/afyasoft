...
/* @var $model frontend\modules\billing\models\NonDrugSearch */
/* @var $form yii\widgets\ActiveForm */
?>

<div class="form-non-drug-search">
    ...
    <?= $form->field($model, 'sale_quote')->widget(\kartik\widgets\Select2::classname(), [
        'data' => \yii\helpers\ArrayHelper::map(\frontend\modules\clinical\models\BlSaleQuote::find()->orderBy('quote_id')->asArray()->all(), 'quote_id', 'quote_id'),
    ...
    <?= $form->field($model, 'service_type')->widget(\kartik\widgets\Select2::classname(), [
        'data' => \yii\helpers\ArrayHelper::map(\frontend\modules\clinical\models\BlServiceType::find()->orderBy('service_type_id')->asArray()->all(), 'service_type_id', 'name'),
    ...
    <?= $form->field($model, 'item_price')->widget(\kartik\widgets\Select2::classname(), [
        'data' => \yii\helpers\ArrayHelper::map(\frontend\modules\clinical\models\BlItemPrice::find()->orderBy('item_price_id')->asArray()->all(), 'item_price_id', 'item_price_id'),
    ...
    <?php /* echo $form->field($model, 'status')->widget(\kartik\widgets\Select2::classname(), [
        'data' => \yii\helpers\ArrayHelper::map(\frontend\modules\clinical\models\BlQuoteStatusCode::find()->orderBy('status_code_id')->asArray()->all(), 'status_code_id', 'name'),
